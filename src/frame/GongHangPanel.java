...
public class GongHangPanel extends JPanel {
	...
	private JTextField cardNoField,nameField,currentTimeField,
		incomeField,//salaryField,salaryDateField
		consumeField,beginAmountField,printOrgField;
	private JButton processBtn;
	
	public GongHangPanel(){
		...
		processBtn.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				if (e.getSource() == processBtn){
					...
					GongHangHandler handler = new GongHangHandler();
					...
					List<GongHangDetail> details = new ArrayList<GongHangDetail>();
					details = handler.generate();
					GongHangExcel excel = new GongHangExcel();
					 Date startTime = new Date();//当前时间
				     SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");    
				     String startStr = sdf.format(startTime);
				     String[] dateStr = startStr.split("-");
				     System.out.println(startStr);
					excel.writeToExcel("d:\\工商银行"+userName+".xls", details,cardNo,userName,printOrg,dateStr[0]+"-"+dateStr[1]+"-"+dateStr[2]);
